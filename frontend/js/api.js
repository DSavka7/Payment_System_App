...
const Api = (() => {
  ...
  async function _fetch(path, options = {}, retry = true) {
    ...
    // Спеціальна обробка 403 для /users/me:
    // Якщо юзер заблокований — сервер повертає {blocked:true, block_reason:..., ...}
    // Ми НЕ кидаємо помилку, а повертаємо цей об'єкт щоб caller міг показати сторінку блокування
    if (res.status === 403 && path === '/users/me') {
      const body = await res.json().catch(() => ({}));
      if (body.blocked === true) {
        return body; // { blocked: true, email, block_reason, message, ... }
      }
    }
  ...
  return {
    ...
    createAccount, getUserAccounts, getAccount, deleteAccount, transfer,
    getAccountTx, getTx,
