/**
 * api.js — оригінальна версія.
 * Використовує /users/ для реєстрації (без двокрокової верифікації).
 * GET /users/me — через окремий ендпоінт.
 */
...
const Api = (() => {
    ...
    // Оригінальна реєстрація — POST /users/ (одноетапна)
    const register = (data) => post('/users/', data);

    const logout   = (rt)   => post('/users/logout', { refresh_token: rt });

    // GET /users/me — повертає профіль поточного користувача
    ...
    const updateAccount   = (id, data) => patch(`/accounts/${id}`, data);
    ...
    return {
        ...
        createAccount, getUserAccounts, getAccount, updateAccount, deleteAccount,
        transfer,
        getAccountTx, getTx,
